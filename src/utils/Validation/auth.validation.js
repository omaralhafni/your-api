...
const submitValidation = (values, handleError) => {
  ...
  if (!values.password) {
    handleError({ email: 'Email address is required', password: 'Password is required' });
  }
}
...
const passwordRegisterValidation = (value, handleError) => {
  if (value === '') {
    handleError({ password: 'Password is required' });
  } else if (value.length < 8 && !/^(?=.*[A-Z])(?=.*\d)[a-zA-Z0-9!@#$%./^&*()_+<>,~`"':;]{8,}$/.test(value)) {
    handleError({ password: "Password should be 8 digits length at least, contains at least one Capital letter, contains at least one number." });
  } else {
    handleError({ password: '' });
  }
}
